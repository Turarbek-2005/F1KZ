...
export default function QualyTable({ year, round }: Props) {
   const args = year && round ? { year, round } : skipToken;
    const { data, isLoading } = useGetYearRoundQualyQuery(args!);
  ...
  if (!data || !data.races?.qualyResults || data.races.qualyResults.length === 0) {
    return (
      <Card>
      ...
      <Card>
        <CardContent>
          <Table>
            ...
            <TableBody>
              {data?.races?.qualyResults.map((result:any) => (
