...
export default function Fp1Table({ year, round }: Props) {
   const args = year && round ? { year, round } : skipToken;
    const { data, isLoading } = useGetYearRoundFp1Query(args!);
  ...
  if (!data || !data.races?.fp1Results || data.races.fp1Results.length === 0) {
    return (
      <Card>
      ...
      <Card>
        <CardContent>
          <Table>
            ...
            <TableBody>
              {data?.races?.fp1Results.filter((result: any) => result.time).map((result:any, index:any) => (
